...
| Inode allocation counter | Yes | `inode_alloc_counter` is a durable key. The numbers left in the crashed node's reserved block are stranded, never reused, and cost nothing in a 64-bit space |
