...
50. **A fence does not re-check that the node is still gone.** `fenceNode` gates on the node being absent, then runs sever → bump → release-arenas → mark with no further check. A node that restarts inside that window is fenced anyway: it comes back healthy with a live lease, is cut off from the device, and is left with a cached `startGen` one behind `clusterGen`, so every write returns `EIO` for the life of the process with nothing reporting it as unhealthy. Four states further on, the same stale fence releases an arena the restarted node has legitimately re-claimed and a peer claims it — two live writers in one range, which is the corruption the design exists to prevent. Found by TLC, not by chaos testing (`FencingAsImplemented`, 136 states). The fix is an **incarnation** check, not a liveness check — TLC rejects the liveness version, since a node can leave, restart, and leave again: capture the membership key's create-revision (or lease ID) at fence start and re-check it before the bump and before `ReleaseArena`. Also `ReleaseArenaID` compares only that the ownership record exists, not which epoch created it. Analysis, traces and the verified fix in `docs/verification/tla-plus.md`.
