...
49. **A directory's link count never counts its subdirectories.** `nlink` is fixed at 2 for a directory's whole life, so a parent that gains a subdirectory stays at 2 where POSIX wants 3. Fixing it means maintaining the count in `mkdir`, `rmdir` and directory `rename`, against a rule the store relies on: a count that never moves is why losing a directory's one name removes it outright. The only assertions still failing (`rename/24.t`).
