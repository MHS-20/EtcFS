...
55. **The inode lock cost a Raft commit on every operation.** Acquire and release around each read and write, at etcd's measured ~2.2 ms per commit — the ceiling no amount of provisioned device IOPS moved, since a serial chain of commits is latency-bound and provisioning buys parallelism.
    ...
    - [x] Lock keys cached past the operation that took them and reused, with node-local exclusion on a per-inode `RWMutex` and a `lock_want:` key by which a blocked peer recalls one — a DLM blocking AST in etcd's terms. An uncontended write is one committed operation, an uncontended read none. A recall demotes the cache entry rather than removing it (removing it split the node-local exclusion in two, since the entry *is* the mutex), honours a 10 ms minimum hold time (without it, contention on one inode cost a recall per operation, worse than the acquire it replaced), and a self-fence drops every cached key at once rather than at process exit.
...
53. **`readdir` was not checked by any consistency model** — CLOSED. Directory listings are the only operation that observes a name's *absence* directly, and nothing checked them: the namespace model saw only `lookup` and the mutations. Closed by decoding both `READDIR` and `READDIRPLUS` and checking each page as what it actually is — a contiguous run in sorted order, since the listing comes out of an etcd prefix scan — so a name that sorts inside a page's range but is missing from it has been dropped, while names past its end stay unconstrained because they are on a later page. A page at offset 0 additionally bounds the directory from below. `test/verify/audit_test.go` covers both directions, and `TestIntegration_ReaddirDecodesWhatTheHandlerEncoded` validates the decoder against the real handler in both framings. `provision_cluster` now runs one `ls` per node so chaos histories contain listings at all.
54. **The docker chaos harness reported infrastructure failures as product failures** — CLOSED. Three defects, all surfacing as the same misleading "ERROR: etcfs-fuse1 never mounted" a minute after the real problem: `teardown_cluster` ran `docker compose down -v` **backgrounded and never waited on**, so one run's teardown outlived its script and deleted the containers the *next* run's provision had just created; `docker compose up -d --build` had both its output and its exit code discarded, so a genuinely failed build was invisible; and the wait loop could not tell "the container does not exist" from "it is up but has not mounted", which have unrelated causes and only one of which `dump_logs` can say anything about. Teardown is now synchronous, a failed build fails the run immediately with the tail of the build log, and the two wait failures are reported distinctly. The trigger in practice was a full disk (Docker rootless keeps its data under `~/.local`, and the build cache had reached 14 GB), which the old code could only express as a mount timeout.
