...
That was less than the round-trip count alone predicts, and the reason is visible in this report's own step list: it never counted the reclaim of buried extents. A random-overwrite workload like the one measured here buries an extent on most writes, and each reclaim was a further commit after the main one — so the real cost was closer to 4 commits per write, not 3. Both the reclaim and the lock release have since been folded into the transaction that publishes the write, which takes an overwriting write to **2** committed operations: the lock acquire, and one transaction carrying the extents, the size, the reclaim and the release together. That change has not been benchmarked.
