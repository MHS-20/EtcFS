...
**The invalidation path that blocked them has since been fixed.** On 2026-08-26 the cache-invalidation client stopped serving acknowledged and unacknowledged traffic on one thread, and the two commits below were re-measured on a scratch branch: the 80,000-file untar completed in 1152 s (69.4 files/s) with not one lock left unyielded for want of a page invalidation. That is the head-of-line blocking gone; whether it makes the copy faster than the build without them needs a same-day baseline, which has not been run. See [Design Decisions](../../design-decisions.md#the-create-time-lock-key-was-reverted).
