...
func (w *WAL) writeEntry(e *Entry) error {
	...
	binary.LittleEndian.PutUint64(buf[41:49], 0)
	_, err := w.f.Write(buf)
	if err == nil {
		err = w.f.Sync()
	}
...
func (w *WAL) Replay(cb func(e *Entry) error) error {
	...
	buf := make([]byte, 49)
	lastUncommitted := make(map[string]*Entry)
	...
	for {
		...
		// Workaround: redeclare err in the if
		if e.Committed {
			delete(lastUncommitted, fmt.Sprintf("%d-%d", e.Ino, e.LogicalOff))
		} else {
			lastUncommitted[fmt.Sprintf("%d-%d", e.Ino, e.LogicalOff)] = e
		}
	}

	for _, e := range lastUncommitted {
		if err := cb(e); err != nil {
			return err
		}
	}
