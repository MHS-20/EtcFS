...
static int ipc_call(struct etcfs_ipc *ipc, uint16_t op, const void *req_payload, uint32_t req_len,
                    uint8_t **resp_payload, uint32_t *resp_len)
{
    ...
    if (send_full(ipc->fd, hdr, 6) < 0)
        goto fail;
    if (req_len > 0 && send_full(ipc->fd, req_payload, req_len) < 0)
        goto fail;

    /* recv response header */
    uint8_t rhdr[4];
    if (recv_full(ipc->fd, rhdr, 4) < 0)
        goto fail;
    ...
    if (!rbuf)
        goto fail;
    if (rlen > 0 && recv_full(ipc->fd, rbuf, rlen) < 0) {
        free(rbuf);
        goto fail;
    }
...
static void ec_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
    ...
    memcpy(req_payload + off, name, nlen);
    off += (uint32_t) nlen;
                       ...
                       struct fuse_file_info *fi)
{
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        size_t sz = fuse_add_direntry(req, dbuf + used, size - used, dname, &st, (off_t) d_off);
        if (sz > size - used)
            break;
        used += sz;
    }
...
// RENAME payload:
// [u64:old_parent][u32:old_name_len][old_name][u64:new_parent][u32:new_name_len][new_name][u32:flags]
// Response: [i32:error]
                      ...
                      fuse_ino_t new_parent, const char *new_name, unsigned int flags)
{
    ...
    memcpy(req_payload + off, old_name, olen);
    off += (uint32_t) olen;
    off += wb_u64(req_payload + off, new_parent);
    off += wb_u32(req_payload + off, (uint32_t) nlen);
    memcpy(req_payload + off, new_name, nlen);
    off += (uint32_t) nlen;
                     ...
                     struct fuse_file_info *fi)
{
    ...
    memcpy(req_payload + payload_off, buf, size);
    payload_off += (uint32_t) size;
...
static void ec_symlink(fuse_req_t req, const char *target, fuse_ino_t parent, const char *name)
{
    ...
    memcpy(req_payload + off, target, tlen);
    off += (uint32_t) tlen;
