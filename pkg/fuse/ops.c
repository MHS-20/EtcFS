...
static int ipc_sync(int fd, uint16_t op, const uint8_t *payload, uint32_t plen,
                     uint8_t **resp, uint32_t *rlen)
{
    uint8_t hdr[6];
    hdr[0] = (uint8_t)(op >> 8); hdr[1] = (uint8_t) op;
    hdr[2] = (uint8_t)(plen >> 24); hdr[3] = (uint8_t)(plen >> 16);
    hdr[4] = (uint8_t)(plen >> 8);  hdr[5] = (uint8_t) plen;
    if (send_full(fd, hdr, 6) < 0) return -1;
    if (plen > 0 && send_full(fd, payload, plen) < 0) return -1;

    uint8_t rhdr[4];
    if (recv_full(fd, rhdr, 4) < 0) return -1;
    uint32_t rl = ((uint32_t)rhdr[0]<<24)|((uint32_t)rhdr[1]<<16)|((uint32_t)rhdr[2]<<8)|(uint32_t)rhdr[3];
    uint8_t *rb = malloc(rl > 0 ? rl : 1);
    if (!rb) return -1;
    if (rl > 0 && recv_full(fd, rb, rl) < 0) { free(rb); return -1; }
    *resp = rb; *rlen = rl;
    return 0;
}
...
static int32_t rb_i32(const uint8_t *p, uint32_t *off) { return (int32_t)rb_u32(p, off); }
static void rb_attr(const uint8_t *p, uint32_t *off, struct etcfs_attr *a) {
    a->ino = rb_u64(p, off); a->size = rb_u64(p, off);
    a->blocks = rb_u64(p, off); a->atime = rb_u64(p, off);
    a->mtime = rb_u64(p, off); a->ctime = rb_u64(p, off);
    a->atime_nsec = rb_u32(p, off); a->mtime_nsec = rb_u32(p, off);
    a->ctime_nsec = rb_u32(p, off); a->mode = rb_u32(p, off);
    a->nlink = rb_u32(p, off); a->uid = rb_u32(p, off);
    a->gid = rb_u32(p, off); a->rdev = rb_u32(p, off);
...
static uint32_t wb_u64(uint8_t *buf, uint64_t v) {
    buf[0]=(uint8_t)(v>>56);buf[1]=(uint8_t)(v>>48);buf[2]=(uint8_t)(v>>40);
    buf[3]=(uint8_t)(v>>32);buf[4]=(uint8_t)(v>>24);buf[5]=(uint8_t)(v>>16);
...
static void fill_stat(struct stat *st, const struct etcfs_attr *a) {
    memset(st, 0, sizeof(*st)); st->st_ino = a->ino; st->st_mode = a->mode;
    st->st_nlink = a->nlink; st->st_uid = a->uid; st->st_gid = a->gid;
    st->st_size = (off_t)a->size; st->st_blksize = a->blksize;
    st->st_blocks = (blkcnt_t)a->blocks; st->st_rdev = a->rdev;
    st->st_atime = (time_t)a->atime; st->st_mtime = (time_t)a->mtime;
...
static void ec_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
    if (parent == FUSE_ROOT_ID &&
        ...
        struct fuse_entry_param e; memset(&e, 0, sizeof(e));
        e.ino = FUSE_ROOT_ID; e.attr_timeout = ETCFS_ATTR_TIMEOUT;
        e.entry_timeout = ETCFS_ENTRY_TIMEOUT; e.attr.st_ino = FUSE_ROOT_ID;
        e.attr.st_mode = S_IFDIR | 0755; e.attr.st_nlink = 2;
        e.attr.st_size = 4096; e.attr.st_blksize = 4096;
        fuse_reply_entry(req, &e); return;
    }

    size_t nlen = strlen(name);
    uint8_t payload[12+256]; uint32_t off = 0;
    ...
    uint32_t pos=0; int32_t e=rb_i32(resp,&pos);
    if(e!=0){fuse_reply_err(req,-e);free(resp);return;}
    struct fuse_entry_param ep; memset(&ep,0,sizeof(ep));
    ep.ino=rb_u64(resp,&pos); struct etcfs_attr a; rb_attr(resp,&pos,&a);
    ep.entry_timeout=(double)rb_u32(resp,&pos); ep.attr_timeout=(double)rb_u32(resp,&pos);
    fill_stat(&ep.attr,&a); free(resp);
...
static void ec_getattr(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info *fi)
{
    ...
    if (ino == FUSE_ROOT_ID || ino == 0) {
        ...
        st.st_ino=FUSE_ROOT_ID; st.st_mode=S_IFDIR|0755; st.st_nlink=2;
        st.st_uid=getuid(); st.st_gid=getgid(); st.st_size=4096; st.st_blksize=4096;
        fuse_reply_attr(req, &st, ETCFS_ATTR_TIMEOUT); return;
    }
    uint8_t payload[8]; wb_u64(payload, ino);
    uint8_t *resp; uint32_t rlen;
    if (ipc_sync(FD(ctx), IPC_OP_GETATTR, payload, 8, &resp, &rlen) < 0)
        { fuse_reply_err(req, EIO); return; }
    uint32_t pos=0; int32_t e=rb_i32(resp,&pos);
    if(e!=0){fuse_reply_err(req,-e);free(resp);return;}
    struct etcfs_attr a; rb_attr(resp,&pos,&a); uint32_t to=rb_u32(resp,&pos); free(resp);
    struct stat st; fill_stat(&st,&a);
...
{
    (void)fi;
    uint8_t payload[20]; uint32_t p=0;
    p+=wb_u64(payload+p,ino); p+=wb_u64(payload+p,(uint64_t)off); p+=wb_u32(payload+p,(uint32_t)size);
    ...
    char *dbuf=malloc(bufsz+512); size_t used=0;
    uint64_t off_cookie = (uint64_t)off;
    for (uint32_t i=0;i<count;i++) {
        uint64_t di=rb_u64(resp,&pos); uint32_t nl=rb_u32(resp,&pos);
        char *dn=(char*)(resp+pos); pos+=nl;
        uint32_t dt=rb_u32(resp,&pos); uint64_t d_off=rb_u64(resp,&pos);
        if (d_off <= off_cookie) continue;  /* skip already-returned entries */
        struct stat st; memset(&st,0,sizeof(st)); st.st_ino=di;
        st.st_mode=(dt==4)?(S_IFDIR|0755):(S_IFREG|0644);
        size_t sz=fuse_add_direntry(req,dbuf+used,bufsz-used,dn,&st,(off_t)d_off);
        if(sz>bufsz-used) break;
        used+=sz;
    }
    free(resp); fuse_reply_buf(req,dbuf,used); free(dbuf);
}
...
static void ec_readlink(fuse_req_t req, fuse_ino_t ino)
{
    ...
    uint32_t pos=0; int32_t e=rb_i32(resp,&pos);
    if(e!=0){fuse_reply_err(req,-e);free(resp);return;}
    uint32_t tl=rb_u32(resp,&pos); char *target=(char*)(resp+pos); resp[pos+tl]='\0';
    fuse_reply_readlink(req,target); free(resp);
}
...
static void ec_statfs(fuse_req_t req, fuse_ino_t ino)
{
    ...
    sv.f_blocks=rb_u64(resp,&pos); sv.f_bfree=rb_u64(resp,&pos); sv.f_bavail=rb_u64(resp,&pos);
    sv.f_files=rb_u64(resp,&pos); sv.f_ffree=rb_u64(resp,&pos); sv.f_bsize=rb_u32(resp,&pos);
    sv.f_namemax=rb_u32(resp,&pos); sv.f_frsize=rb_u32(resp,&pos); free(resp);
    fuse_reply_statfs(req,&sv);
}

static void ec_open(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info *fi)
{
    (void)ino; fi->fh=0; fi->direct_io=1; fi->keep_cache=0; fuse_reply_open(req,fi);
}

static void ec_opendir(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info *fi)
{
    (void)ino; fi->fh=0; fuse_reply_open(req,fi);
}
...
{
    size_t nlen=strlen(name);
    uint8_t payload[20+256]; uint32_t off=0;
    off+=wb_u64(payload+off,parent); off+=wb_u32(payload+off,(uint32_t)nlen);
    memcpy(payload+off,name,nlen); off+=(uint32_t)nlen;
    off+=wb_u32(payload+off,(uint32_t)mode); off+=wb_u32(payload+off,(uint32_t)(fi?fi->flags:0));
...
static void ec_mkdir(fuse_req_t req, fuse_ino_t parent, const char *name, mode_t mode)
{
    ...
    off+=wb_u64(payload+off,parent); off+=wb_u32(payload+off,(uint32_t)nlen);
    memcpy(payload+off,name,nlen); off+=(uint32_t)nlen;
    off+=wb_u32(payload+off,(uint32_t)mode); off+=wb_u32(payload+off,022);
...
static void ec_unlink(fuse_req_t req, fuse_ino_t parent, const char *name)
{
    ...
    if (ipc_sync(FD(ctx), IPC_OP_UNLINK, payload, off, &resp, &rlen) < 0)
        { fuse_reply_err(req, EIO); return; }
    int32_t e=0; if(rlen>=4) e=(int32_t)((uint32_t)resp[0]<<24|(uint32_t)resp[1]<<16|(uint32_t)resp[2]<<8|(uint32_t)resp[3]);
    free(resp); fuse_reply_err(req, e!=0?-e:0);
}
                      ...
                      fuse_ino_t new_parent, const char *new_name, unsigned int flags)
{
    ...
    uint8_t payload[40+512]; uint32_t off=0;
    off+=wb_u64(payload+off,old_parent); off+=wb_u32(payload+off,(uint32_t)olen);
    memcpy(payload+off,old_name,olen); off+=(uint32_t)olen;
    off+=wb_u64(payload+off,new_parent); off+=wb_u32(payload+off,(uint32_t)nlen);
...
{
    (void)fi;
    uint8_t *payload=malloc(20+size); uint32_t pos=0;
    pos+=wb_u64(payload+pos,ino); pos+=wb_u64(payload+pos,(uint64_t)off);
    pos+=wb_u32(payload+pos,(uint32_t)size); memcpy(payload+pos,buf,size); pos+=(uint32_t)size;

    uint8_t *resp; uint32_t rlen;
    int ret=ipc_sync(FD(ctx), IPC_OP_WRITE, payload, pos, &resp, &rlen); free(payload);
    if(ret<0){fuse_reply_err(req,EIO);return;}
    uint32_t p=0; int32_t e=rb_i32(resp,&p); uint32_t written=rb_u32(resp,&p); free(resp);
...
static void ec_setattr(fuse_req_t req, fuse_ino_t ino, struct stat *attr, int to_set,
                       struct fuse_file_info *fi)
{
    uint8_t payload[20+84]; uint32_t off=0;
    off+=wb_u64(payload+off,ino); off+=wb_u64(payload+off,(fi?fi->fh:0));
    off+=wb_u32(payload+off,(uint32_t)to_set); (void)attr;
...
static void ec_symlink(fuse_req_t req, const char *target, fuse_ino_t parent, const char *name)
{
    ...
    uint8_t payload[20+512]; uint32_t off=0;
    off+=wb_u64(payload+off,parent); off+=wb_u32(payload+off,(uint32_t)nlen);
    memcpy(payload+off,name,nlen); off+=(uint32_t)nlen;
    off+=wb_u32(payload+off,(uint32_t)tlen); memcpy(payload+off,target,tlen); off+=(uint32_t)tlen;
...
static void ec_link(fuse_req_t req, fuse_ino_t ino, fuse_ino_t new_parent, const char *new_name)
{
    ...
    uint8_t payload[24+256]; uint32_t off=0;
    off+=wb_u64(payload+off,ino); off+=wb_u64(payload+off,new_parent);
    off+=wb_u32(payload+off,(uint32_t)nlen); memcpy(payload+off,new_name,nlen); off+=(uint32_t)nlen;
...
static void ec_mknod(fuse_req_t req, fuse_ino_t parent, const char *name, mode_t mode, dev_t rdev)
{
    ...
    off+=wb_u64(payload+off,parent); off+=wb_u32(payload+off,(uint32_t)nlen);
    memcpy(payload+off,name,nlen); off+=(uint32_t)nlen;
    off+=wb_u32(payload+off,(uint32_t)mode); off+=wb_u32(payload+off,(uint32_t)rdev);
...
static void ec_getlk(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info *fi, struct flock *lock)
    { (void)ino;(void)fi; struct flock lk=*lock; lk.l_type=F_UNLCK; fuse_reply_lock(req,&lk); }
...
struct fuse_lowlevel_ops *etcfs_fuse_ops(void)
{
    ...
    ops.lookup = ec_lookup; ops.getattr = ec_getattr; ops.readdir = ec_readdir;
    ops.readlink = ec_readlink; ops.statfs = ec_statfs; ops.open = ec_open;
    ops.opendir = ec_opendir; ops.create = ec_create; ops.mkdir = ec_mkdir;
    ops.unlink = ec_unlink; ops.rmdir = ec_rmdir; ops.rename = ec_rename;
    ops.symlink = ec_symlink; ops.link = ec_link; ops.setattr = ec_setattr;
    ops.write = ec_write; ops.mknod = ec_mknod;
    ops.release = ec_release; ops.releasedir = ec_releasedir;
    ops.flush = ec_flush; ops.fsync = ec_fsync; ops.fsyncdir = ec_fsyncdir;
    ops.read = ec_read; ops.getlk = ec_getlk; ops.setlk = ec_setlk;
