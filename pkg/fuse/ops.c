...
void
etcfs_ipc_destroy(struct etcfs_ipc *ipc)
{
    if (ipc) {
        close(ipc->fd);
        free(ipc);
...
static void ec_readdir(fuse_req_t req, fuse_ino_t ino, size_t size,
...
{
    (void)parent; (void)name; (void)newparent; (void)newname; (void)flags;
    fuse_reply_err(req, EROFS);
}
...
static void ec_link(fuse_req_t req, fuse_ino_t ino, fuse_ino_t newparent,
                    const char *newname)
{
    (void)ino; (void)newparent; (void)newname;
    fuse_reply_err(req, EROFS);
}
...
{
    (void)req; (void)ino; (void)bufv; (void)off; (void)fi;
    fuse_reply_err(req, EROFS);
}
...
{
    (void)ino; (void)fi; (void)lock; (void)sleep;
    fuse_reply_err(req, 0);
}
...
{
    (void)parent; (void)name; (void)mode; (void)rdev;
    fuse_reply_err(req, EROFS);
}
