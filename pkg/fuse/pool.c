...
static int send_full(int fd, const void *buf, size_t len)
{
    ...
    while (len > 0) {
        ...
        if (n < 0) {
            if (errno == EINTR)
                continue;
            etcfs_log(ETCFS_LOG_ERROR, "send_full write error fd=%d errno=%d", fd, errno);
            return -1;
        }
        len -= (size_t) n;
        p += n;
...
static int do_ipc_exchange(int fd, uint16_t opcode, const void *payload, uint32_t plen,
                           uint8_t **resp_payload, uint32_t *resp_len)
{
    ...
    if (!rbuf)
        return -1;
    if (rlen > 0 && recv_full(fd, rbuf, rlen) < 0) {
        free(rbuf);
        return -1;
    }
...
static void *ipc_worker_thread(void *arg)
{
    ...
    for (;;) {
        ...
        uint8_t *resp = NULL;
        uint32_t rlen = 0;
        etcfs_log(ETCFS_LOG_INFO, "IPC worker exchange start op=%u fd=%d plen=%u", req->opcode,
                  w->fd, req->plen);
