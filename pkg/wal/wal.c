...
struct etcfs_wal *
etcfs_wal_open(const char *path)
{
...
void
etcfs_wal_close(struct etcfs_wal *wal)
{
    if (wal) {
        if (wal->fd >= 0) {
...
static int
wal_write_entry(struct etcfs_wal *wal, uint8_t flags,
                uint64_t ino, uint64_t log_off, uint64_t disk_off,
                uint64_t len, uint64_t gen, uint64_t ts)
{
    uint8_t buf[WAL_ENTRY_SIZE];
    size_t pos = 0;

    buf[pos++] = flags;
    memcpy(buf + pos, &ino, 8);      pos += 8;
    memcpy(buf + pos, &log_off, 8);  pos += 8;
    memcpy(buf + pos, &disk_off, 8); pos += 8;
    memcpy(buf + pos, &len, 8);      pos += 8;
    memcpy(buf + pos, &gen, 8);      pos += 8;
    memcpy(buf + pos, &ts, 8);       pos += 8;
...
int
etcfs_wal_append(struct etcfs_wal *wal, const struct wal_entry *entry)
{
    if (!wal || wal->fd < 0) return -1;

    int ret = wal_write_entry(wal, entry->committed ? WAL_FLAG_COMMITTED : 0,
                              entry->ino, entry->logical_off, entry->disk_off,
...
int
etcfs_wal_mark_committed(struct etcfs_wal *wal, uint64_t ino,
                         uint64_t logical_off)
{
    ...
    return wal_write_entry(wal, WAL_FLAG_COMMITTED,
                           ino, logical_off, 0, 0, 0, wall_time_ns());
}

int
etcfs_wal_replay(struct etcfs_wal *wal, wal_replay_cb cb, void *userdata)
{
    if (!wal || wal->fd < 0) return -1;
...
int
etcfs_wal_truncate_before(struct etcfs_wal *wal, uint64_t timestamp_ns)
{
    (void)wal;
    (void)timestamp_ns;
